...
const TravelForm = ({ darkMode, setDarkMode }) => {
  ...
  return (
    <div
      className={`flex flex-col px-4 py-10 justify-between min-h-screen ${
        darkMode ? "bg-[#0B0809] text-white" : "bg-white text-gray-900"
      }`}
    >
      <div
        className={`w-full max-w-3xl mx-auto ${
          darkMode ? "bg-[#0B0809]" : "bg-white"
        } rounded-lg relative`}
      >
      ...
      <div
        className={`flex justify-center mb-3 sm:justify-start mt-auto ${
          darkMode ? "bg-" : ""
        }`}
      >
        <div className="w-full sm:max-w-xs">
          <Button onClick={handleSubmit} darkMode={darkMode}>
            Continue
          </Button>
        </div>
      </div>
